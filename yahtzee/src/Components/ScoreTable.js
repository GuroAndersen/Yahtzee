...
export default function ScoreTable({numPlayers}) {
  ...
  const upperSection = ["Ones", "Twos", "Threes", "Fours", "Fives", "Sixes", "Bonus"];
  const lowerSection = ["Three of a kind", "Four of a kind", "Full house", "Small straight", "Large straight", "Yahtzee", "Chance", "Yahtzee Bonus"];
              ...
              {Array.from({ length: cols }).map((_, j) => {
                if (j === 0) {
                  return <td key={j}>{allSections[i]}</td>;
                    ...
                    if (i === 0) {
                        return <td key={j}>Player {j}'s Score</td>;
                      } else {
